<script lang="ts">
  ...
  import { disconnectEdge } from './object-authoring'
  ...
  $: top = Math.max(8, Math.min(y, window.innerHeight - (inspecting ? 560 : 310)))
  ...
  async function disconnectCurrent() {
    ...
    const action = edge.affordances.find(item => item.id === 'edge.delete') ?? null
    if (!action?.enabled) {
      mutationError = action?.reason ?? 'Disconnect is not available for this edge.'
      return
    }
    if (!window.confirm(`Disconnect ${srcName} → ${dstName}? This removes the persisted edge; it is not the same as Dismiss.`)) return
    mutationBusy = true
    mutationError = null
    try {
      await disconnectEdge(edge.id)
      onClose()
    } catch (err) {
...
<div class="menu" bind:this={menuEl} style="left: {left}px; top: {top}px;" role="menu">
  ...
  {#if inspecting}
    <section class="inspect-panel" aria-label="Edge inspector">
      ...
      {/if}
      {#if mutationError}<div class="mutation-error" role="status">{mutationError}</div>{/if}
      {#if mutationBusy}<div class="mutation-status">Disconnecting…</div>{/if}
      <EdgeAuthoringSection {edge} onDisconnect={() => void disconnectCurrent()} />
    </section>
  {/if}
...
<style>
  .menu {
    ...
    border: 1px solid var(--hairline); border-radius: 6px; background: rgba(12,23,38,.98);
  ...
  .mutation-error, .mutation-status { margin: 7px 0; padding: 6px 7px; border-radius: 4px; font: 9px/1.35 var(--font-mono); }
  .mutation-error { border: 1px solid rgba(238,123,115,.38); color: var(--crimson); background: rgba(238,123,115,.06); }
  .mutation-status { border: 1px solid var(--hairline-2); color: var(--muted); background: var(--panel-2); }
</style>
