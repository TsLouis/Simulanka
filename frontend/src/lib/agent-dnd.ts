...
function normalizeRefs(values: unknown[]): AgentDragRef[] {
...
export function writeAgentDragRefs(event: DragEvent, refs: AgentDragRef[]): void {
  ...
  if (!transfer || normalized.length === 0) return
  transfer.effectAllowed = 'copy'
  transfer.setData(AGENT_REF_MIME, JSON.stringify(normalized))
  ...
  transfer.setData(
    ...
    normalized.length === 1
      ? normalized[0].label
      : `${normalized.length} Simulanka objects`,
  )
}
...
export function readAgentDragRefs(event: DragEvent): AgentDragRef[] {
  ...
  try {
    ...
    return normalizeRefs(Array.isArray(parsed) ? parsed : [parsed])
  } catch {
    return []
  }
}
