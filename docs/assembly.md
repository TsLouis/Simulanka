...
| `status` | proposed / accepted（边）；planned / done（experiment）；open / supported / refuted（claim） | 生命周期 |
...
**🔲 `evidence extract <run>`（切片③，`run end` 内也触发）**：
