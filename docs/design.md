...
- **通用对话入口（2026-07-08 定向，实现推迟）**：DiscussPanel 从「核对分歧」单场景推广为前端通用
  ...
  上下文重放；前端对话视图复用 node-edge 视觉语言（图皮、文件芯）。③ **锚定式交互（同日晚补定）**：
  **取消全局自由聊天窗**——一切对话从画布上的选择集（一个或多个节点/边）发起并绑定其上，人机不空谈；
  无自然实体锚时用容器节点（experiment / plan 目录 / 根）兜底。§13.6 批次核对讨论由此成为「锚定在
  分歧边集合上」的特例（「一批一场」的全局上下文理由保留）。对话 UI 面本身不消失（打字/读回复仍要
  面板），消失的是无锚形态——面板永远是某个选择集的附属物。余下岔口收窄为：写权范围、沉淀物、UI 具体形态。
...
**完备性演进方向**（2026-07-08 定）：plan 块的表达力目标对齐 **§5.1 原子系统**（非裸 kernel ops）——
凡原子与语义边能表达的研究叙事，块应最终能表达。v1 已知缺口：分析者铸 `evidence`（引外部文献）、
已有实体间补 `addresses`/`tests` 边、自由 `note`。**不预补**——真实轮次首撞者驱动（补法均为
distill.edges / plan 段的小扩词，格式版本随演进）。
